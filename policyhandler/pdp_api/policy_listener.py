    ...
    def _on_pdp_message(self, *args):
        ...
        try:
            ...
            if message.get(ADDED):
                message = message.get(ADDED)
            policies_updated = [
                {PDP_POLICY_ID: message.get(ID),
                 POLICY_VERSION: message.get(POLICY_VER)}

            ]

            if message.get(REMOVED):
                message=message.get(REMOVED)

            policies_removed = [
                {PDP_POLICY_ID: message.get(ID),
                 PDP_POLICY_VERSION: message.get(POLICY_VER)}
