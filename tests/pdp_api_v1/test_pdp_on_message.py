...
class TestPdpMessage(unittest.TestCase):
    ...
    def test_pdp_message(self):
        ...
        message = notification.mock_notification()
        expected_updated_policy, expected_removed_policy = notification.mock_policy_list()
        audit = Audit(job_name="test pdp_on_message", req_message="start _on_pdp_message testing")
        ...
        message = notification.mock_multiple_notification()
        PolicyListener(audit, _policy_updater)._on_pdp_message(message)
        expected_updated_policy, expected_removed_policy = notification.mock_multiple_policy_list()
        self.assertTrue(mock_update.called)
        mock_update.assert_called_with(expected_updated_policy, expected_removed_policy)
