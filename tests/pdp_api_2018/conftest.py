...
def _auto_setup_policy_engine_2018():
    """initialize the mock-policy-engine_2018 per the whole test session"""
    _LOGGER.info("create _auto_setup_policy_engine_2018")
    MockPolicyEngine2018.init()
    yield _auto_setup_policy_engine_2018
    _LOGGER.info("teardown _auto_setup_policy_engine_2018")


@pytest.fixture(scope="module")
def fix_pdp_api_2018():
    """test on the old (2018) pdp API"""
    _LOGGER.info("setup fix_pdp_api_2018 %s", MockSettings.OLD_PDP_API_VERSION)
    MockSettings.setup_pdp_api(MockSettings.OLD_PDP_API_VERSION)

    yield fix_pdp_api_2018
    MockSettings.setup_pdp_api()
    _LOGGER.info("teardown fix_pdp_api_2018 %s", MockSettings.OLD_PDP_API_VERSION)
...
def fix_policy_receiver_websocket(monkeypatch):
    ...
    monkeypatch.setattr('policyhandler.pdp_api_2018.policy_listener.websocket', MockWebSocket)
...
def fix_select_latest_policies_boom(monkeypatch):
    ...
    policy_utils_path = 'policyhandler.pdp_api_2018.policy_utils.PolicyUtils'
